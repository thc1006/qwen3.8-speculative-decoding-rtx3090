...
| if **H2'** (quantization × arithmetic intensity) holds | speculative arms **below** baseline, falling further as n-max rises | speculative arms **above** baseline |
...
| **B** | drafted-token vs rejected-token cost (p-min × n-max) | 7 | 5 | ~2.3 h |
| **C** | method breadth on one host: drafter-quant ladder, classic draft, ngram family, DSpark | ~10 | 3 | ~2 h |
| **L** | does speculation survive the ~25× decode collapse past ~80 K ([#27623](https://github.com/ggml-org/llama.cpp/issues/27623))? | 4 × depths | 3 | ~2 h |
...
| **X** | thinking on/off × method, multimodal, concurrency | — | — | later |
...
| `bw-lo` | −400 | 0 | 420 W | bandwidth ↓ |
| `bw-hi` | +400 | 0 | 420 W | bandwidth ↑ |
| `pw-lo` | 0 | 0 | 250 W | compute ↓ |
| `pw-vlo` | 0 | 0 | 175 W | compute ↓↓ |
