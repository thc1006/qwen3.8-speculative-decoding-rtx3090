...
> at two rungs of four, which is every rung a 24 GB card can hold.
...
| **Qs** | does the bf16 anchor #25618 rests on actually hold, and does #26750's CUDA acceptance figure reproduce on a second CUDA architecture? | **running.** Qwen3.5-9B-MTP on the same card, four rungs Q4_K_M / Q6_K / Q8_0 / **BF16** -- the anchor Qwen3.8-27B structurally cannot reach, since its bf16 is 50 GB and fits on neither card here. Unlike Phase Q these are one uniform-quantization family, so bit width is **not** confounded with scheme, which makes this the cleaner of the two ladders on that axis. `Q4_K_M` is the exact model and quant of [#26750](https://github.com/ggml-org/llama.cpp/issues/26750), which reports MTP acceptance collapsing to 35.8-40.7 % on CUDA against ~92 % on Vulkan; the `mtp-n6` arm is its matched configuration on sm_86. Hypotheses registered in Correction 21, before the BF16 rung existed and before any acceptance figure was read |
