...
> Phase M is complete: 1575 records, 0 incidents, both targets in one session. Phase Q is running.
...
| **Q** | does the target quantization ladder move the marginal cost per verified position? | **one rung of four complete**: UD-Q4_K_XL, 300 records, 0 incidents, and it replicates the same arms three other sessions measured on the same file to within 1.4 pp (n2 +60.2 %, n3 +52.4 %, n5 +32.0 %). The remaining rungs are **blocked on disk**, not on design: UD-Q5_K_XL needs 33 GB free and 26 GB is, because Correction 9 holds the 22 GB MoE target while the Phase M anchor is unresolved. Releasing it unblocks two more rungs and gives up chasing that anchor. Corrections 10, 11 |
