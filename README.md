...
> association.
> **Running:** Phase B, `--spec-draft-n-max` crossed with `--spec-draft-p-min`, 21 arm-passes.
> **Blocked:** Phase V. The run loop exists now; both vLLM MTP arms fail to load on this card
> because the MTP module allocates its own bf16 `embed_tokens` and `lm_head`, 2.37 GiB each, on
> top of a 17.33 GiB target. Filed as
...
widths printed here should be read as potentially under-covering rather than as exact 95 %
statements. Those coverage figures come from three synthetic data-generating processes, not from
this data's own unknown distribution, so they diagnose the estimator rather than quantify this
interval; the primary Phase A effects sit far from zero under that diagnostic either way.
...
- **Phase M's cost telemetry, and its exclusions.** The `mean_len` figure every cost quantity in
  ...
  the analyser computes an intention-to-treat series it does not yet tabulate. What would settle
  the first is llama.cpp exposing `n_draft_verif_steps`, which `server_slot_stats` already holds
  and `to_json()` does not publish.
