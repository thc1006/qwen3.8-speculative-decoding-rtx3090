...
**Contents** - [every document in this repository](docs/README.md) |
[Scope](#how-this-study-is-scoped-and-what-it-inherits) |
[What this is not claiming](#what-this-is-not-claiming) |
...
[Evidence status](#evidence-status) | [Design](#design) | [Later phases](#later-phases) |
[Reproduce](#reproduce) | [Limitations](#limitations)

## How this study is scoped, and what it inherits
...
Verify width is `n-max + 1`, the number of positions one target forward pass scores, which is
why the `n2` arm sits at width 3.
...
Four symbols, defined once. **`mean_len`** is tokens emitted per target forward pass. **`k`** is
what one speculative cycle costs in units of a plain decode step; **`k0`** is its intercept and
**`c`** its slope per extra verified position. **`w`**, the verification width, is `n-max + 1`:
the number of positions one target forward pass scores. They are defined here because the
generated table of what each phase may not claim uses all four without them having been said.
...
MTP refits to **0.2954** and the difference is **-0.0473 [-0.0489, -0.0456]**. That middle value
matters to anyone checking the arithmetic: 0.2904 and 0.2481 differ by 0.0423, and the reported
0.0473 is the matched-width comparison, not the difference of the two headline coefficients.

That difference is between two **configurations**,
...
not shared between the two methods over the widths both cover.

A configured n-max of 8 puts the
...
| E4 | 450 records over 9 files, complete, 0 incidents | control -- Phase E3's two 420 W arms re-run with idle held around the sampling window, so both of its ends sit in the same steady state. Committed data already said the offset is per-window rather than per-second -- in the 35 file-arm cells with enough internal spread to split, where the long third of the windows is at least 1.5x the short third, the window grows 1.81x and the offset 1.01x -- but window length varies inside a cell because prompts generate at different speeds, which is a correlation with the window and not a manipulation of it. This manipulates it. The power traces are recorded, so `power.draw` can be deconvolved against `power.draw.instant` and the averaging width read off directly rather than quoted |
| E5 | 225 records, complete, 0 incidents | control -- one baseline arm at three power caps, with Phase E4's 4 s roll and traces. The cap sets how far the card climbs above its idle-with-model draw of 125 to 131 W, so it sets the step: 287.4 W at 420, 122.1 at 250 and 26.5 at 150, a range of 10.8x against the 1.4x the committed records happen to span on their own. Three passes over three arms so the rotation closes and each cap visits each order position exactly once -- E3 and E4 could leave arm order fixed because their estimand was within-arm, and this one is between arms |
| E6 | 225 records over 9 files, complete, 0 incidents | control -- one arm at the stock 420 W cap so the step is held, with the generation length moved instead -- 200, 400 and 800 tokens, giving windows of about 9.0, 13.9 and 23.2 s at a step that varies 1.7 %. Phase E5's only lever was the power cap, which sets the step and the generation rate together at a Spearman of -0.917, so its split into a step-scaled part and a fixed part is a property of the model chosen (Correction 50). The 400-token cell reproduces E5's top cap -- span 13.89 against 13.91, step 286.3 against 287.4 -- so the two phases measure the same object and the models it cannot separate coincide there and diverge either side of it |
...
| **thermal gate at arm entry** | on identical non-speculative work the card loses **1.95 % of SM clock** between the first and second arm of a pass, and the per-record extreme over a whole pass is 9.3 % (1950 -> 1769 MHz) -- but that wider figure mixes position with arm, since the arms differ in power draw. The 1.95 % is what a rotation has to defeat, and it is larger than several effects under study. Measured on the card **as found**, 450 W and overclocked, and not re-measured at the 420 W stock cap the primary matrix ran on: it establishes that the drift exists, not its size there. `docs/METHODOLOGY_AUDIT.md` A9 |
...
- **Energy magnitude is provisional.** Energy comes from sampled board-power telemetry and a
  separately measured prefill calibration. The audits support the direction. The magnitude is not
  calibrated: an external power meter is the reference that would settle it. Phases E3 to E6
  bounded the instrument rather than the card -- the driver's cumulative counter and the
  instantaneous sensor agree to 0.15 % while both depart from the averaged field every figure here
  integrates, which moves the headline to **-36.3 %** on a matched pair -- and what is left is a
  fixed term of **1.4 to 3.6 J** whose size depends on a model none of those phases can choose
  between. The measurements, the two withdrawn readings and what each phase may not be used to
  claim: [`docs/ENERGY.md`](docs/ENERGY.md).
- **The energy figures are not an identical-answer comparison.** Most speculative generations
  follow a different token trajectory from the baseline they are compared against, and semantic
  quality is not scored, so this is not energy for the same answer.
...
| | what it is |
|---|---|
| **Archived release** | Zenodo **v1.0.4**, [`10.5281/zenodo.22246518`](https://doi.org/10.5281/zenodo.22246518), from commit `01a76b76c2...`, deposited 2026-09-02 and checked against `git archive v1.0.4`: 1048 files, every SHA-256 equal. v1.0.0 stays archived at [`10.5281/zenodo.22149942`](https://doi.org/10.5281/zenodo.22149942); a deposit is a record of what was published and is not withdrawn when it is superseded. |
| **Concept DOI** | [`10.5281/zenodo.22149941`](https://doi.org/10.5281/zenodo.22149941) — resolves to whichever version is newest, which today is v1.0.4. Cite this one unless you need to pin. |
| **Current source** | signed tag `v1.0.4`, which is the deposited tree. A tag is cheap and revocable and a Zenodo record is neither, so the deposit stays a separate deliberate step and `repro/DEPOSITS.json` records which tags have had one: three of the five release tags have not, and `phase-a-v1` is listed there too because it is not a release at all. `git rev-parse v1.0.4^{}` gives the commit. |
| **Previous tag** | signed tag `v1.0.3`, commit `b384cadcdb...`. Tagged as the intended deposit and superseded before one was made: Correction 58 found `TODO.md` stating that no scrambled embedding had been observed while the probe that observes it sat in the same archive, deposited at v1.0.0. |
| **Tag before that** | signed tag `v1.0.2`, commit `8d301e8d64...`. Superseded the same way: 13 numbered corrections were appended after it, 45 through 57, and Correction 57 withdrew the negative rejection-cost claim from Phase A and Phase KV, so depositing that tree would have archived a reading this study no longer makes. |
| **And before that** | signed tag `v1.0.1`, commit `dba40dcf78...`. **Tagged, pushed, and deliberately not deposited**: an external review found seven correctness blockers in it, and the tag is kept as the record of that rather than deleted. Corrections 42 and 43. |
