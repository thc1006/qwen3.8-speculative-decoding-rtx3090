...
# f16 variant of phase_l for the 48 GB card. One line differs from phase_l.py, the KV type,
# which is the whole point: every rung the 24 GB card runs uses q8_0, so a cliff seen there
# cannot be separated from the cache precision that made room for the depth. The VRAM table is
# recomputed for f16: 64 KV layers x ctx x 4 KV heads x 512 x 2 bytes, plus 16.35 GiB of weights
# and about 2 for the compute buffer.
